...
public class Lick : InputAction {
    ...
    public override void RespondToInput(GameMaster gameMaster, string[] separatedInputWords) {
        if(separatedInputWords.Length >= 2) {
            gameMaster.LogStringWithReturn(gameMaster.TestVerbDictionaryWithNoun(gameMaster.interactableItems.lickDictionary, separatedInputWords[0], separatedInputWords[1]));
        }
        else {
